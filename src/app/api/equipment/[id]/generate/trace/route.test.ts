...
describe("POST /api/equipment/:id/generate/trace", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.findGeneratedDocument
      ...
      .mockResolvedValueOnce({
        ...
        currentContent: JSON.stringify({
          test_cases: [
            { test_id: "IOQ-001", linked_req_ids: ["URS-001"], linked_risk_ids: ["RA-001"] }
          ]
        })
      })
      .mockResolvedValueOnce({
        ...
        currentContent: JSON.stringify({
          test_cases: [
            { test_id: "OQ-001", linked_req_ids: ["URS-001"], linked_risk_ids: ["RA-001"] }
          ]
        })
      })
      .mockResolvedValueOnce(null);
    mocks.findLatestJob.mockResolvedValue({ id: "job-1" });
    mocks.createGeneratedDocument.mockResolvedValue({ id: "tm-1" });
  ...
  it("generates TM and persists version hash", async () => {
    const response = await POST(new Request("http://localhost", { method: "POST" }), {
      params: Promise.resolve({ id: "machine-1" })
    });
